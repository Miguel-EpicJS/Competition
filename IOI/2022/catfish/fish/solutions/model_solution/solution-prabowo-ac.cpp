...
long long max_weights(int N, int M, vector<int> X, vector<int> Y, vector<int> W) {
  vector<vector<pair<int, long long>>> fish(N + 2);
  for (int i = 0; i < M; ++i) {
  ...
  auto getRowWeight = [&](int row, int col) {
    return (upper_bound(fish[row].begin(), fish[row].end(), make_pair(col, LLONG_MAX)) - 1)->second;
  };

  vector<vector<int>> indices(N + 2);
  indices[0] = {-1};
  for (int i = 1; i <= N; ++i) {
    for (pair<int, int> f : fish[i - 1]) {
  ...
  vector<long long> dpInc = {0LL}, dpDec = {0LL};
  for (int i = 1; i <= N + 1; ++i) {
    vector<long long> ndpInc(indices[i].size());
    long long pref = 0LL;
    int pidx = 0;
    for (int j = 0; j < static_cast<int>(indices[i].size()); ++j) {
      int col = indices[i][j];
      while (pidx < static_cast<int>(indices[i - 1].size()) && indices[i - 1][pidx] <= col) {
        pref = max(pref, dpInc[pidx] - getRowWeight(i - 1, indices[i - 1][pidx]));
        ++pidx;
      }
      ndpInc[j] = max(pref + getRowWeight(i - 1, col), dpDec[0]);
    }

    vector<long long> ndpDec(indices[i].size());
    long long suff = 0;
    ...
    for (int j = static_cast<int>(indices[i].size()) - 1; j >= 0; --j) {
      ...
      while (sidx > 0 && indices[i - 1][sidx - 1] >= col) {
        --sidx;
        suff = max(suff, max(dpInc[sidx], dpDec[sidx]) + getRowWeight(i, indices[i - 1][sidx]));
      }
